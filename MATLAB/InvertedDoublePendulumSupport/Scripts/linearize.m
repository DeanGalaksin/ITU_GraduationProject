function linearize()
%LINEARIZE - Sets the model to be open loop and generates the linearized mod
%            model. The linearized model is put in the base workspace as
%            the variable sys_cart_dpen.

% Copyright 2023 The MathWorks, Inc.

mdl = 'InvertedDoublePendulum';
...
% Linearize about inverted upright position
[A,B,C,D] = linmod(mdl,X,0);

% Remove the discrete states from the state space model
A = A(1:6,1:6);
B = B(1:6);
C = C(:,1:6);
sys_cart_dpen.A = A;
